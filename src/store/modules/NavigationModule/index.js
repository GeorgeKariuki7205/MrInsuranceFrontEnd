...
const actions = {
  ...
  postingTheDataForCoverSearch({
    ...
    obj["subCategoryId"] =
      state.navigationState[state.cover].subCategories[state.subCategory].id;
    ...
    axios
      .post(
        "https://mrinsuranceapi.georgekprojects.tk/api/insuranceCovers",
        obj
      )
      .then((response) => {
        if (response.status === 200) {
          ...
          response.data.forEach((element) => {
            switch (element.cover.route_name) {
              case "Health":
                ...
                combinedHealthFinancialBreakDown[
                  element.uuid
                ] = healthFinancialBreakDown;
                break;
              case "Motor":
                ...
                combinedMotorFinancialBreakDownObject[
                  element.uuid
                ] = financial_break_down_array;
          ...
          if (caseObtained === "Motor") {
            commit(
              "UPDATING_THE_FINANCIAL_BREAKDOWN",
              combinedMotorFinancialBreakDownObject
            );
          ...
          commit(
            "UPDATING_ADDITIONAL_COVERS_STATE",
            additionalCoversInitialStates
          );
        }
      })
      .catch((error) => {
    ...
    if (activity === "remove") {
      for (const key in financialBreakDownStateDefined) {
        if (financialBreakDownStateDefined[key].uuid === obj.uuid) {
          if (
            financialBreakDownStateDefined[key].additionId === obj.additionId
          ) {
  ...
  activatingAdditionalCovers({
    ...
    if (state.additionalCoversPremiumState[data.premiumUUID] === null) {
      ...
      for (const key in state.additionalCoversPremiumState[data.premiumUUID]) {
        allAdditionalCovers[counter] =
          state.additionalCoversPremiumState[data.premiumUUID][key];
        counter = counter + 1;
      }
      additionalCover["additionalId"] = data.additionalId;
      additionalCover["premiumUUID"] = data.premiumUUID;
      ...
      if (payableAmount === data.premiumUUId) {
        if (data.functionality === "subtract") {
          newPayableAmount[payableAmount] =
            state.payableAmountState[payableAmount] - data.cost;
        } else {
          newPayableAmount[payableAmount] =
            state.payableAmountState[payableAmount] + data.cost;
        }
        ...
        newPayableAmount[payableAmount] =
          state.payableAmountState[payableAmount];
      }
    }
  ...
  updateTheAdditionalCover({
    ...
    if (
      Object.keys(state.additionalCoversPremiumState[obj.premium]).length === 0
    ) {
  ...
  updateAdditionalCovresAddedOrRemoved() {
    ...
    for (const premium in premiums) {
      ...
      // var count = 0;
      var count2 = 0;
      var currentAddedAdditionals =
        state.additionalCoversPremiumState[premiumUUID];
      ...
      for (const additionalCover in premiums[premium].additionalCovers) {
        ...
        for (const additionalCoverSelected in currentAddedAdditionals) {
          if (
            currentAddedAdditionals[additionalCoverSelected].premiumUUID ===
            premiumUUID
          ) {
            if (
              premiums[premium].additionalCovers[additionalCover]["id"] !==
              currentAddedAdditionals[additionalCoverSelected]["additionalId"]
            ) {
              ++checker;
            }
          }
        ...
        if (checker >= count2) {
          newOtherAdditional[counter] =
            premiums[premium].additionalCovers[additionalCover];
  ...
  sendingPaymentRequest({
    ...
    axios
      .post("https://mrinsuranceapi.georgekprojects.tk/api/stkPush", obj)
      .then((response) => {
        if (response.status === 200) {
          ...
          if (response.data.ResponseCode === "0") {
            ...
            state.paymentDetailsGetter["CheckoutRequestID"] =
              response.data.CheckoutRequestID;
            state.paymentDetailsGetter["MerchantRequestID"] =
            ...
            objectToSendRequestForPayment["MerchantRequestID"] =
              response.data.MerchantRequestID;
            objectToSendRequestForPayment["CheckoutRequestID"] =
              response.data.CheckoutRequestID;
            objectToSendRequestForPayment["visitorId"] = state.visitorIdState;
            objectToSendRequestForPayment["amountPayable"] =
              objectRecieved.cost;
            objectToSendRequestForPayment["insuranceCoverID"] =
              objectRecieved.insuranceCoverID;

            // ! sending post request.

            axios
              .post(
                "https://mrinsuranceapi.georgekprojects.tk/api/intentionToPay",
                objectToSendRequestForPayment
              )
              .then((response) => {
                if (response.status === 200) {
